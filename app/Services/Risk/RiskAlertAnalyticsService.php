...
class RiskAlertAnalyticsService
{
     ...
     * Read-only: no writes, no caching changes, Eloquent only.
     */
    public function metrics(string $countryId): array
    {
        // ── Event counts ──────────────────────────────────────────────────
        $triggeredCount = RiskAlertEvent::where('country_id', $countryId)
            ->whereIn('event_type', ['triggered', 'retriggered'])
            ->count();

        $retriggeredCount = RiskAlertEvent::where('country_id', $countryId)
            ->where('event_type', 'retriggered')
            ->count();

        $resolvedCount = RiskAlertEvent::where('country_id', $countryId)
            ->where('event_type', 'resolved')
            ->count();

        $acknowledgedCount = RiskAlertEvent::where('country_id', $countryId)
